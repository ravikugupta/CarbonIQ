...
import { Target, CheckCircle2, Flame, Trophy, Share2, X } from 'lucide-react';
...
export default function ActionTracker() {
  ...
  useEffect(() => {
    ...
    if (data) {
      const storedStats = data.stats || { totalPoints: 0, totalCo2Saved: 0, streak: 0, lastActionDate: null };
      const storedDate = data.completedDate || null;
      const storedCompleted = data.completedActions || [];

      // Reset completed actions if they were logged on a previous day
      if (storedDate && storedDate !== today()) {
        setCompletedActions([]);
        setCompletedDate(today());
      } else {
        setCompletedActions(storedCompleted);
        setCompletedDate(storedDate);
      }
  ...
  const saveState = (newStats, newCommitments, newCompleted, newDate) => {
  ...
  const toggleCommitment = (id) => {
    ...
    saveState(stats, newCommits, completedActions, completedDate);
  };
  ...
  const completeAction = (action) => {
    ...
    const newStats = {
      ...
      totalCo2Saved: parseFloat((stats.totalCo2Saved + action.co2Saved).toFixed(2)),
      streak: newStreak,
    ...
    saveState(newStats, commitments, newCompleted, todayStr);
  };

  const shareProgress = async () => {
    const text = `I'm tracking my carbon footprint on CarbonIQ! I've saved ${stats.totalCo2Saved} kg of CO₂ and earned ${stats.totalPoints} points with a ${stats.streak}-day streak. Join me!`;
    if (navigator.share) {
      try {
        await navigator.share({ title: 'My CarbonIQ Progress', text });
      } catch (e) {
  ...
  return (
    <div className="space-y-8 animate-fadeIn py-8 max-w-5xl mx-auto">
      ...
      {toast && (
        <div
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-gray-900 border border-gray-700 text-white px-5 py-3 rounded-xl shadow-2xl"
          role="status"
          aria-live="polite"
        >
          <span>{toast}</span>
      ...
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
            <Target className="text-primaryGreen" size={32} aria-hidden="true" />
            Action Tracker
          </h1>
      ...
      {/* Stats */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6" role="region" aria-label="Your stats">
        <div className="bg-[#0f1915] border border-gray-800 rounded-xl p-6 flex flex-col items-center justify-center shadow-xl">
          <Trophy className="text-yellow-400 mb-2" size={36} aria-hidden="true" />
          <h3 className="text-gray-400 text-sm font-medium uppercase">Total Points</h3>
          <span className="text-4xl font-bold text-white" aria-label={`${stats.totalPoints} total points`}>{stats.totalPoints}</span>
        </div>
        <div className="bg-[#0f1915] border border-gray-800 rounded-xl p-6 flex flex-col items-center justify-center shadow-xl">
          <Target className="text-green-400 mb-2" size={36} aria-hidden="true" />
          <h3 className="text-gray-400 text-sm font-medium uppercase">CO₂ Saved</h3>
          <span className="text-4xl font-bold text-white" aria-label={`${stats.totalCo2Saved.toFixed(1)} kilograms CO₂ saved`}>
            {stats.totalCo2Saved.toFixed(1)} <span className="text-xl text-gray-500">kg</span>
          </span>
        </div>
        <div className="bg-[#0f1915] border border-gray-800 rounded-xl p-6 flex flex-col items-center justify-center shadow-xl">
          <Flame className="text-orange-400 mb-2" size={36} aria-hidden="true" />
          <h3 className="text-gray-400 text-sm font-medium uppercase">Day Streak</h3>
          <span className="text-4xl font-bold text-white" aria-label={`${stats.streak} day streak`}>{stats.streak}</span>
        </div>
      </div>

      {/* Action Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" role="list" aria-label="Eco actions">
        {ECO_ACTIONS.map(action => {
          ...
          return (
            <article
              key={action.id}
              role="listitem"
              aria-label={`${action.title} — ${action.category}`}
              className={`p-5 rounded-xl border transition-all ${isCompleted ? 'bg-primaryGreen/10 border-primaryGreen/30' : 'bg-[#0f1915] border-gray-800 hover:border-gray-600'}`}
            >
              ...
              <div className="flex gap-2 mt-auto">
                {!isCompleted ? (
                  ...
                  <div
                    className="w-full py-2 bg-primaryGreen/20 text-primaryGreen rounded-lg text-sm font-medium flex items-center justify-center gap-2"
                    role="status"
                    aria-label={`${action.title} completed today`}
                  >
